...
class AutoClientUnitTest(unittest.TestCase):
    ...
    def test_can_move_to_diagonal_room(self):
        ...
        move_response = self.player.take_turn(game_state)
        expected_response = {'move': {'_location': 'Conservatory', 'player': 'p03'}}
    ...
    def test_p01_answers_p04_has_one_card_suggested(self):
        ...
        # {‘move_completed’: True, ’answer’: {‘from_player’: ‘p02’, ‘card’: ‘Candlestick’}}

        game_state = {'move_completed': True, 'answer': {'card': 'Plum', 'from_player': 'p01'}}
    ...
    def test_1_in_c1_for_card_should_clear_c2_for_card_in_all_other_player_sub_tables(self):
        ...
        game_state = {'answer': {'has_card': True, 'from_player': 'p02'},
                      'suggestion': ['White', 'Kitchen', 'Revolver']}
        self.player.update(game_state)

        suggestion = {'answer': {'has_card': True, 'from_player': 'p02'},
                      'suggestion': ['White', 'Ballroom', 'Pipe']}
        self.player.update(game_state)

        suggestion = {'answer': {'has_card': True, 'from_Player': 'p02'},
                      'suggestion': ['White', 'Conservatory', 'Candlestick']}
        self.player.update(game_state)

        # get the player sub-tables in p04's _pad for testing before and after the action condition
        p01_tbl = self.player._pad.get_player_table('p01')
        p02_tbl = self.player._pad.get_player_table('p02')
        p03_tbl = self.player._pad.get_player_table('p03')
        ...
        self.assertTrue(self.player._pad.get_player_table('p01').c1.White, np.nan)
        self.assertTrue(self.player._pad.get_player_table('p02').c1.White, np.nan)
        self.assertTrue(self.player._pad.get_player_table('p03').c1.White, np.nan)
        self.assertTrue(self.player._pad.get_player_table('p04').c1.White, np.nan)
    ...
    def test_p01_tells_other_player_has_a_suggested_card_all_other_computer_players_mark_their_pads(self):
        ...
        from the suggestion.
        """
        game_state = {'answer': {'has_card': True, 'from_player': 'p01'},
                      'suggestion': ['Mustard', 'Kitchen', 'Revolver']}

        first_card = game_state['suggestion'][0]
        second_card = game_state['suggestion'][1]
        third_card = game_state['suggestion'][2]
    ...
    def test_already_a_one_in_c2_for_one_of_the_cards_suggested_so_append_two_to_all_three_answered_card_cells(self):
        ...
        game_state = {'answer': {'has_card': True, 'from_player': 'p03', 'cards': {'Mustard', 'Kitchen', 'Rope'}}}
